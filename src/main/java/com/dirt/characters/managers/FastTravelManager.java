...
import com.dirt.api.TravelDestinationProvider;
import com.dirt.characters.data.FastTravelPoint;
import com.dirt.characters.data.TravelDestination;
import com.dirt.characters.inventory.impl.travel.FastTravelTransitionGUI;
import com.dirt.characters.scheduler.ScheduledTask;
...
public class FastTravelManager {
    ...
    public FastTravelManager(DirtCharacters plugin) {
        this.plugin = plugin;
    ...
    /**
     * Destination categories contributed by the rest of the suite — owned
     * properties and governed regions from DirtNations, workplaces from DirtBiz.
     * Categories with nothing in them for this player are dropped, so the screen
     * only offers travel that will actually work.
     */
    public List<TravelCategory> getExternalCategories(UUID playerUuid) {
        List<TravelCategory> categories = new ArrayList<>();
        for (TravelDestinationProvider provider : DirtApi.getTravelDestinationProviders()) {
            List<TravelDestination> destinations = new ArrayList<>();
            try {
                for (TravelDestinationProvider.Destination d : provider.getDestinations(playerUuid)) {
                    if (d.getLocation() == null || d.getLocation().getWorld() == null) continue;
                    destinations.add(new TravelDestination(d.getName(), d.getDescription(), d.getLocation(), null));
                }
            } catch (Exception e) {
                plugin.getLogger().warning("[FastTravel] Destination provider "
                        + provider.getClass().getName() + " failed: " + e.getMessage());
                continue;
            }
            if (destinations.isEmpty()) continue;
            categories.add(new TravelCategory(provider.getCategoryName(), provider.getIconMaterial(),
                    provider.getCategoryLore(), destinations));
        }
        return categories;
    }

    /** One provider-contributed category, resolved for a specific player. */
    public static final class TravelCategory {
        private final String name;
        private final String iconMaterial;
        private final List<String> lore;
        private final List<TravelDestination> destinations;

        TravelCategory(String name, String iconMaterial, List<String> lore, List<TravelDestination> destinations) {
            this.name = name;
