package com.dirt.characters.managers;
...
public class CharacterManager {
    ...
    public CharacterManager(DirtCharacters plugin, IDManager idManager) {
        this.plugin = plugin;
        this.idManager = idManager;
        this.dataFolder = new File(plugin.getDataFolder(), "data/characters");
    ...
    public CharacterMutationResult createCharacter(UUID uuid, CharacterProfile profile) {
        if (uuid == null) return CharacterMutationResult.failure(CharacterMutationResult.Status.INVALID_ARGUMENT);
        if (characterCache.containsKey(uuid)) return CharacterMutationResult.failure(CharacterMutationResult.Status.ALREADY_EXISTS);
        if (!isValidProfile(profile)) return CharacterMutationResult.failure(CharacterMutationResult.Status.INVALID_PROFILE);

        CharacterData data = new CharacterData();
        data.setPlayerUuid(uuid);
        applyProfile(data, profile);
        long now = System.currentTimeMillis();
        data.setBirthDate(now);
        data.setFirstJoinDate(now);
        data.setAlive(true);
        saveCharacter(data);
        clearPendingCreation(uuid);
        removeForcedCreation(uuid);
        return CharacterMutationResult.success(new com.dirt.characters.api.CharacterViewImpl(data));
    }

    public CharacterMutationResult updateCharacter(UUID uuid, CharacterProfile profile) {
        if (uuid == null) return CharacterMutationResult.failure(CharacterMutationResult.Status.INVALID_ARGUMENT);
        CharacterData data = characterCache.get(uuid);
        if (data == null) return CharacterMutationResult.failure(CharacterMutationResult.Status.NOT_FOUND);
        if (!isValidProfile(profile)) return CharacterMutationResult.failure(CharacterMutationResult.Status.INVALID_PROFILE);
    ...
    public CharacterMutationResult removeCharacter(UUID uuid) {
        if (uuid == null) return CharacterMutationResult.failure(CharacterMutationResult.Status.INVALID_ARGUMENT);
        CharacterData data = characterCache.get(uuid);
        if (data == null) return CharacterMutationResult.failure(CharacterMutationResult.Status.NOT_FOUND);
    ...
    public CharacterMutationResult updateCharacterBio(UUID uuid, String bio) {
        if (uuid == null) return CharacterMutationResult.failure(CharacterMutationResult.Status.INVALID_ARGUMENT);
        CharacterData data = characterCache.get(uuid);
        if (data == null) return CharacterMutationResult.failure(CharacterMutationResult.Status.NOT_FOUND);
        if (bio == null) return CharacterMutationResult.failure(CharacterMutationResult.Status.INVALID_BIO);
        String normalizedBio = bio.trim();
        if (normalizedBio.length() > plugin.getSettings().getCharacterBioMaxLength()) {
            return CharacterMutationResult.failure(CharacterMutationResult.Status.INVALID_BIO);
        }
        data.setBio(normalizedBio);
        saveCharacter(data);
        return CharacterMutationResult.success(new com.dirt.characters.api.CharacterViewImpl(data));
    }
    ...
    public void saveCharacter(CharacterData data) {
        ...
        config.set("homeLocation", data.getHomeLocation());
        config.set("bio", data.getBio());
    ...
    private boolean isValidProfile(CharacterProfile profile) {
        return profile != null
                && isValidNamePart(profile.getFirstName())
                && isValidNamePart(profile.getMiddleName())
                && isValidNamePart(profile.getLastName())
                && ("MALE".equalsIgnoreCase(profile.getGender()) || "FEMALE".equalsIgnoreCase(profile.getGender()));
    }

    private boolean isValidNamePart(String value) {
        return value != null && value.trim().matches("[A-Za-z][A-Za-z'-]{0,23}");
    }

    private void applyProfile(CharacterData data, CharacterProfile profile) {
        data.setFirstName(normalizeName(profile.getFirstName()));
        data.setMiddleName(normalizeName(profile.getMiddleName()));
        data.setLastName(normalizeName(profile.getLastName()));
        data.setGender(profile.getGender().trim().toUpperCase(Locale.ROOT));
    }
    ...
    private CharacterData readCharacterFile(UUID uuid, File file) {
        ...
        data.setHomeLocation(config.getString("homeLocation"));
        data.setBio(config.getString("bio", ""));
