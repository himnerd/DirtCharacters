...
import com.dirt.characters.util.LText;
...
public class ContractCreationGUI extends InventoryGUI {
    ...
    public ContractCreationGUI(DirtCharacters plugin) {
        this.plugin = plugin;
    }

    public ContractCreationGUI(DirtCharacters plugin, java.util.UUID playerUuid) {
        this.plugin = plugin;
    }

    public ContractCreationGUI(DirtCharacters plugin, ContractData draft) {
        this.plugin = plugin;
    ...
    static void promptBodyInput(DirtCharacters plugin, ContractData draft, Player player, boolean append) {
        ...
        plugin.getChatInputManager().requestInput(player, prompt, input -> {
            LText.handleInput(plugin, player, input, text -> {
                ...
                plugin.getPlatformScheduler().runAtEntity(player, () -> plugin.getGUIManager().openGUI(new ContractCreationGUI(plugin, draft), player));
            }, () -> plugin.getPlatformScheduler().runAtEntity(player, () -> plugin.getGUIManager().openGUI(new ContractCreationGUI(plugin, draft), player)));
        });
    }
