...
import com.dirt.characters.inventory.impl.ServerCharacterListGUI;
import com.dirt.characters.inventory.impl.contract.ContractCreationGUI;
...
public class DirtCCommand implements CommandExecutor, TabCompleter {
    ...
    public DirtCCommand(DirtCharacters plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length > 0 && args[0].equalsIgnoreCase("create")) {
            if (plugin.getCharacterManager().hasCharacter(player.getUniqueId())) {
                // Ending a character is DirtLife's job; without it there is
                // nothing to offer beyond saying the slot is taken.
                if (DirtApi.life() != null) {
                    ...
                    DirtApi.life().openLifeMenu(player);
                } else {
                    player.sendMessage("§eYou already have an active character.");
                }
                return true;
            }
